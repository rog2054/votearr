...
- [X] Removal of orphan votes from the db (ie when the associated media no longer exists in Radarr). This could be done as part of the 'ready_to_delete' deletion function, however ideally would also detect and tidy if the media was deleted separately outside of Votearr.
...
- [ ] Deploy the Flask application behind a WSGI server with Nginx as a reverse proxy.
